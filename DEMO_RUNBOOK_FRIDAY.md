...
**Yes — automated in this POC when configured as below:** use StackSet auto-deployment to your workload OU(s) and set discovery to org-wide mode (`DISCOVER_ALL_ORG_ACCOUNTS=true`). Then newly added ACTIVE org accounts are discovered on the next schedule without manually editing `SPOKE_ACCOUNTS`.
...
**Set once for full auto-account pickup**

1. Deploy StackSet to OU targets with `--auto-deployment Enabled=true,RetainStacksOnAccountRemoval=false`.
2. In `db-discovery` env vars set:
   - `DISCOVER_ALL_ORG_ACCOUNTS=true`
   - `ORG_SKIP_MANAGEMENT_ACCOUNT=true`
   - optional `ORG_EXCLUDE_ACCOUNT_IDS=...`
3. Keep EventBridge schedule enabled.
