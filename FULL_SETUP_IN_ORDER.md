...
New EC2 instances (SSM Online) appear on the **next** scheduled run.  
For **new AWS accounts**, use this automation-first path to avoid manual env var edits:

- Deploy spoke bootstrap StackSet to target OUs (Step 13).
- Enable Lambda org discovery: `DISCOVER_ALL_ORG_ACCOUNTS=true`.
- Keep `ORG_SKIP_MANAGEMENT_ACCOUNT=true` and optional `ORG_EXCLUDE_ACCOUNT_IDS`.

With this setup, newly vended **ACTIVE** org accounts in target OUs are discovered on the next schedule run automatically (once StackSet resources finish in that account).
...
4. For full onboarding automation, target OUs (not static account lists) and prefer:
   - `DISCOVER_ALL_ORG_ACCOUNTS=true`
   - `ORG_SKIP_MANAGEMENT_ACCOUNT=true`
   - `SPOKE_ACCOUNTS` left empty (or used only for non-org exceptions)
