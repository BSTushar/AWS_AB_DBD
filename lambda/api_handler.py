...
def _regions_declared_in_env():
    out = []
    for key in ("INVENTORY_UI_REGIONS", "DISCOVERY_REGIONS"):
...
def merged_inventory_regions(items):
    from_s3 = [i.get("region") for i in items if isinstance(i, dict) and i.get("region")]
    return sorted(set(from_s3 + _regions_declared_in_env()))
...
def apply_record_filters(items, qs):
    ...
    db_id_q = _norm_text(qs.get("db_id"))
    ...
    for i in items:
        ...
        if instance_q and i.get("instance_id") != instance_q:
            continue
        if db_id_q and _norm_text(i.get("db_id")) != db_id_q:
            continue
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if path_segments and path_segments[-1].lower() == "regions" and "accounts" not in path.lower():
            ...
            regions = merged_inventory_regions(items)
