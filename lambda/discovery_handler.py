...
DISCOVERY_INCLUDE_STOPPED_EC2 = os.environ.get("DISCOVERY_INCLUDE_STOPPED_EC2", "true").lower() in ("1", "true", "yes")
...
def get_linux_ec2_instance_ids(ec2_client):
    out = []
    ...
    try:
        ...
        logger.warning("DescribeInstances (linux inventory) failed: %s", e)
    return list(dict.fromkeys(out))


def build_stub_records_offline_ec2(account_id, region, instance_ids, instance_details):
    rows = []
    ts = datetime.utcnow().isoformat() + "Z"
    for iid in instance_ids:
        info = instance_details.get(iid, {})
        ...
        rows.append(
            {
                ...
                "discovery_status": "skipped_offline",
                "region": region,
                "ec2_state": info.get("ec2_state", "unknown"),
            }
        )
    return rows
...
def lambda_handler(event, context):
    ...
    for account_id in accounts_to_scan:
        ...
        for region in regions:
            ...
            online_ids = [i[0] for i in online_instances]

            offline_only_ids = []
            if DISCOVERY_INCLUDE_STOPPED_EC2:
                ec2_linux_ids = get_linux_ec2_instance_ids(ec2)
                online_set = set(online_ids)
                offline_only_ids = [i for i in ec2_linux_ids if i not in online_set]
            else:
                ec2_linux_ids = []

            if not online_ids and not offline_only_ids:
                logger.info(
                    "No Linux EC2 (non-terminated) in account %s region %s",
                    account_id,
                    region,
                )
                continue

            union_ids = list(dict.fromkeys(online_ids + offline_only_ids))
            instance_details = get_instance_details(ec2, union_ids)

            if offline_only_ids:
                logger.info(
                    "Account %s %s: %s SSM-Online, %s EC2-only (offline/stub)",
                    account_id,
                    region,
                    len(online_ids),
                    len(offline_only_ids),
                )
                all_records.extend(
                    build_stub_records_offline_ec2(account_id, region, offline_only_ids, instance_details)
                )
